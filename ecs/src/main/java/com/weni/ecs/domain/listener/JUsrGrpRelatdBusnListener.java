/**
 * JUsrGrpRelatdBusn Entity Listener 클래스
 *
...
@Slf4j
public class JUsrGrpRelatdBusnListener {
    ...
    //----------------------------------------------------
    @PostLoad
    public void onPostLoad(JUsrGrpRelatdBusn postData) {
        log.info("onPostLoad : Select후 호출({})" , postData.toString() );
    }

    @PrePersist
    public void onPrePersist(JUsrGrpRelatdBusn preData) {
        log.info("onPrePersist : Insert전 호출({})" , preData.toString() );
    }

    @PostPersist
    public void onPostPersist(JUsrGrpRelatdBusn postData) {
        log.info("onPrePersist : Insert후 호출({})" , postData.toString() );
    }

    @PreUpdate
    public void onPreUpdate(JUsrGrpRelatdBusn preData) {
        log.info("onPreUpdate : Update전 호출({})" , preData.toString() );
    }

    @PostUpdate
    public void onPostUpdate(JUsrGrpRelatdBusn postData) {
        log.info("onPostUpdate : Update후 호출({})" , postData.toString() );
    }

    @PreRemove
    public void onPreRemove(JUsrGrpRelatdBusn preData) {
        log.info("onPreRemove  : Delete전 호출({})" , preData.toString() );
    }

    @PostRemove
    public void onPostRemove(JUsrGrpRelatdBusn postData) {
