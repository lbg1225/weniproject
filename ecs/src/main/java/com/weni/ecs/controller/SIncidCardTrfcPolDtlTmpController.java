/**
 * SIncidCardTrfcPolDtlTmp Entity Controller 클래스
 *
...
import com.weni.ecs.domain.entity.SIncidCardTrfcPolDtlTmpPK;
import com.weni.ecs.domain.repository.SIncidCardTrfcPolDtlTmpRepository;
...
@RestController
@Tag(name = "SIncidCardTrfcPolDtlTmp 사건 카드 교통 경찰 상세 TMP(Table) Controller")
@Slf4j
public class SIncidCardTrfcPolDtlTmpController {

    @Autowired
    SIncidCardTrfcPolDtlTmpRepository repository;


    @Autowired
    ObjectMapper mapper;

    // @Operation(summary = "사건 카드 교통 경찰 상세 TMP(Table) 전체조회" , summary = "사건 카드 교통 경찰 상세 TMP(Table) 조회" )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Found the sincidcardtrfcpoldtltmps", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = SIncidCardTrfcPolDtlTmp.class)) }),
            @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content) })
    @GetMapping("/sincidcardtrfcpoldtltmps")
    public ResponseEntity<Page<SIncidCardTrfcPolDtlTmp>> getAll(@PageableDefault Pageable pageable) {
        try {
			log.info("findAll");
    ...
    // @Operation(summary = "사건 카드 교통 경찰 상세 TMP(Table) Key조회" , description = "사건 카드 교통 경찰 상세 TMP(Table) Primary Key로 조회" )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Found the SIncidCardTrfcPolDtlTmp", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = SIncidCardTrfcPolDtlTmp.class)) }),
            @ApiResponse(responseCode = "404", description = "SIncidCardTrfcPolDtlTmp not found", content = @Content) })
    @GetMapping("/sincidcardtrfcpoldtltmps/{recptNum}/{dtlDivCode}/{dtlSeqNum}")
    public ResponseEntity<SIncidCardTrfcPolDtlTmp> getById(@PathVariable("recptNum") Integer recptNum, @PathVariable("dtlDivCode") String dtlDivCode, @PathVariable("dtlSeqNum") Integer dtlSeqNum) {
        Optional<SIncidCardTrfcPolDtlTmp> data = repository.findById(new SIncidCardTrfcPolDtlTmpPK(recptNum, dtlDivCode, dtlSeqNum));
    ...
    // @Operation(summary = "사건 카드 교통 경찰 상세 TMP(Table) 등록" , description = "사건 카드 교통 경찰 상세 TMP(Table) 신규 데이터 등록" )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Create the SIncidCardTrfcPolDtlTmp", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = SIncidCardTrfcPolDtlTmp.class)) }),
            @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content) })
    @PostMapping("/sincidcardtrfcpoldtltmps")
    ResponseEntity<SIncidCardTrfcPolDtlTmp> postData(@RequestBody SIncidCardTrfcPolDtlTmp newData) {
        try {
    ...
    // @Operation(summary = "사건 카드 교통 경찰 상세 TMP(Table) 수정" , description = "사건 카드 교통 경찰 상세 TMP(Table) 데이터 수정" )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Update the SIncidCardTrfcPolDtlTmp", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = SIncidCardTrfcPolDtlTmp.class)) }),
            @ApiResponse(responseCode = "404", description = "SIncidCardTrfcPolDtlTmp not found", content = @Content) })
    @PutMapping("/sincidcardtrfcpoldtltmps/{recptNum}/{dtlDivCode}/{dtlSeqNum}")
    ResponseEntity<SIncidCardTrfcPolDtlTmp> putData(@RequestBody SIncidCardTrfcPolDtlTmp newData, @PathVariable("recptNum") Integer recptNum, @PathVariable("dtlDivCode") String dtlDivCode, @PathVariable("dtlSeqNum") Integer dtlSeqNum) {
        return repository.findById(new SIncidCardTrfcPolDtlTmpPK(recptNum, dtlDivCode, dtlSeqNum)) //
                .map(oldData -> {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Update the SIncidCardTrfcPolDtlTmp", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = SIncidCardTrfcPolDtlTmp.class)) }),
            @ApiResponse(responseCode = "404", description = "SIncidCardTrfcPolDtlTmp not found", content = @Content) })
	@PatchMapping("/sincidcardtrfcpoldtltmps/{recptNum}/{dtlDivCode}/{dtlSeqNum}")
    ResponseEntity<SIncidCardTrfcPolDtlTmp> patchData(@RequestBody Map<String, Object> newMap, @PathVariable("recptNum") Integer recptNum, @PathVariable("dtlDivCode") String dtlDivCode, @PathVariable("dtlSeqNum") Integer dtlSeqNum) {
        SIncidCardTrfcPolDtlTmp newData = mapper.convertValue(newMap, SIncidCardTrfcPolDtlTmp.class);
        return repository.findById(new SIncidCardTrfcPolDtlTmpPK(recptNum, dtlDivCode, dtlSeqNum)) //
                .map(oldData -> {
    ...
    // @Operation(summary = "사건 카드 교통 경찰 상세 TMP(Table) 삭제" , description = "사건 카드 교통 경찰 상세 TMP(Table) Primary Key로 삭제" )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Delete the SIncidCardTrfcPolDtlTmp", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = SIncidCardTrfcPolDtlTmp.class)) }),
            @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content) })
    @DeleteMapping("/sincidcardtrfcpoldtltmps/{recptNum}/{dtlDivCode}/{dtlSeqNum}")
    public ResponseEntity<HttpStatus> deleteUser(@PathVariable("recptNum") Integer recptNum, @PathVariable("dtlDivCode") String dtlDivCode, @PathVariable("dtlSeqNum") Integer dtlSeqNum) {
        try {
            repository.deleteById(new SIncidCardTrfcPolDtlTmpPK(recptNum, dtlDivCode, dtlSeqNum));
