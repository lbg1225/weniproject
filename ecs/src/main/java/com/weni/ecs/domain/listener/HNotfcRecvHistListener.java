/**
 * HNotfcRecvHist Entity Listener 클래스
 *
...
@Slf4j
public class HNotfcRecvHistListener {
    ...
    //----------------------------------------------------
    @PostLoad
    public void onPostLoad(HNotfcRecvHist postData) {
        log.info("onPostLoad : Select후 호출({})" , postData.toString() );
    }

    @PrePersist
    public void onPrePersist(HNotfcRecvHist preData) {
        log.info("onPrePersist : Insert전 호출({})" , preData.toString() );
    }

    @PostPersist
    public void onPostPersist(HNotfcRecvHist postData) {
        log.info("onPrePersist : Insert후 호출({})" , postData.toString() );
    }

    @PreUpdate
    public void onPreUpdate(HNotfcRecvHist preData) {
        log.info("onPreUpdate : Update전 호출({})" , preData.toString() );
    }

    @PostUpdate
    public void onPostUpdate(HNotfcRecvHist postData) {
        log.info("onPostUpdate : Update후 호출({})" , postData.toString() );
    }

    @PreRemove
    public void onPreRemove(HNotfcRecvHist preData) {
        log.info("onPreRemove  : Delete전 호출({})" , preData.toString() );
    }

    @PostRemove
    public void onPostRemove(HNotfcRecvHist postData) {
