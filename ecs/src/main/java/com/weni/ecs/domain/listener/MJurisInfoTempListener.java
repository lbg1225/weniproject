/**
 * MJurisInfoTemp Entity Listener 클래스
 *
...
@Slf4j
public class MJurisInfoTempListener {
    ...
    //----------------------------------------------------
    @PostLoad
    public void onPostLoad(MJurisInfoTemp postData) {
        log.info("onPostLoad : Select후 호출({})" , postData.toString() );
    }

    @PrePersist
    public void onPrePersist(MJurisInfoTemp preData) {
        log.info("onPrePersist : Insert전 호출({})" , preData.toString() );
    }

    @PostPersist
    public void onPostPersist(MJurisInfoTemp postData) {
        log.info("onPrePersist : Insert후 호출({})" , postData.toString() );
    }

    @PreUpdate
    public void onPreUpdate(MJurisInfoTemp preData) {
        log.info("onPreUpdate : Update전 호출({})" , preData.toString() );
    }

    @PostUpdate
    public void onPostUpdate(MJurisInfoTemp postData) {
        log.info("onPostUpdate : Update후 호출({})" , postData.toString() );
    }

    @PreRemove
    public void onPreRemove(MJurisInfoTemp preData) {
        log.info("onPreRemove  : Delete전 호출({})" , preData.toString() );
    }

    @PostRemove
    public void onPostRemove(MJurisInfoTemp postData) {
