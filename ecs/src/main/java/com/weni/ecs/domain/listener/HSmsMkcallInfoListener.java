/**
 * HSmsMkcallInfo Entity Listener 클래스
 *
...
@Slf4j
public class HSmsMkcallInfoListener {
    ...
    //----------------------------------------------------
    @PostLoad
    public void onPostLoad(HSmsMkcallInfo postData) {
        log.info("onPostLoad : Select후 호출({})" , postData.toString() );
    }

    @PrePersist
    public void onPrePersist(HSmsMkcallInfo preData) {
        log.info("onPrePersist : Insert전 호출({})" , preData.toString() );
    }

    @PostPersist
    public void onPostPersist(HSmsMkcallInfo postData) {
        log.info("onPrePersist : Insert후 호출({})" , postData.toString() );
    }

    @PreUpdate
    public void onPreUpdate(HSmsMkcallInfo preData) {
        log.info("onPreUpdate : Update전 호출({})" , preData.toString() );
    }

    @PostUpdate
    public void onPostUpdate(HSmsMkcallInfo postData) {
        log.info("onPostUpdate : Update후 호출({})" , postData.toString() );
    }

    @PreRemove
    public void onPreRemove(HSmsMkcallInfo preData) {
        log.info("onPreRemove  : Delete전 호출({})" , preData.toString() );
    }

    @PostRemove
    public void onPostRemove(HSmsMkcallInfo postData) {
