/**
 * MClssfyCodeGetNum Entity Listener 클래스
 *
...
@Slf4j
public class MClssfyCodeGetNumListener {
    ...
    //----------------------------------------------------
    @PostLoad
    public void onPostLoad(MClssfyCodeGetNum postData) {
        log.info("onPostLoad : Select후 호출({})" , postData.toString() );
    }

    @PrePersist
    public void onPrePersist(MClssfyCodeGetNum preData) {
        log.info("onPrePersist : Insert전 호출({})" , preData.toString() );
    }

    @PostPersist
    public void onPostPersist(MClssfyCodeGetNum postData) {
        log.info("onPrePersist : Insert후 호출({})" , postData.toString() );
    }

    @PreUpdate
    public void onPreUpdate(MClssfyCodeGetNum preData) {
        log.info("onPreUpdate : Update전 호출({})" , preData.toString() );
    }

    @PostUpdate
    public void onPostUpdate(MClssfyCodeGetNum postData) {
        log.info("onPostUpdate : Update후 호출({})" , postData.toString() );
    }

    @PreRemove
    public void onPreRemove(MClssfyCodeGetNum preData) {
        log.info("onPreRemove  : Delete전 호출({})" , preData.toString() );
    }

    @PostRemove
    public void onPostRemove(MClssfyCodeGetNum postData) {
